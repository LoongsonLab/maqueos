...
int sys_input(char *buf) // 从键盘获取
{
    ...
    {
        buf[i++] = read_queue.buffer[read_queue.tail];  // 将字符存入 buf
        read_queue.tail = (read_queue.tail + 1) & (BUFFER_SIZE - 1); // 更新 tail
        read_queue.count--;  // 减少 count
    }
