...
int sys_read(int fd, char *buf)
{
	...
	int blockCount =(file->inode->size%BLOCK_SIZE == 0)? (file->inode->size /BLOCK_SIZE): (file->inode->size /BLOCK_SIZE + 1);
	for(int i=0; i<blockCount; i++){
		read_inode_block(file->inode, i, buf+BLOCK_SIZE*i, BLOCK_SIZE);
	}
	return 0;
}
int sys_write(int fd, char *buf)
{
	...
	int blockCount =(buf_len%BLOCK_SIZE == 0)? (buf_len/BLOCK_SIZE): (buf_len /BLOCK_SIZE + 1);
	for(int i = 0; i <blockCount; i++){
