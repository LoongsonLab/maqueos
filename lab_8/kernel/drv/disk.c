...
struct buffer *find_buffer(short blocknr,int type)
{
	int i;
	...
	for (i = 0; i < NR_BUFFER; i++)
	{
		if (buffer_table[i].blocknr == blocknr){
			buffer_table[i].seq=i;
			buffer_table[i].type=type;
			buffer_table[i].used=current->pid;
...
struct buffer *get_buffer(short blocknr,int type)
{
	int i;
	...
	buffer_table[0].seq=0;
	buffer_table[0].type=type;
	buffer_table[0].used=current->pid;
...
void write_block(short blocknr, char *buf)
{
	...
	lock_disk();
	bf = find_buffer(blocknr,1);
	if (!bf)
		bf = get_buffer(blocknr,1);
...
char *read_block(short blocknr)//blocknr数据块号
{
	struct buffer *bf;

	lock_disk();
	bf = find_buffer(blocknr,0);
	if (bf)
	{
		unlock_disk();
	...
	bf = get_buffer(blocknr,0);//块号，type
	write_detail();
