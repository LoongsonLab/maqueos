...
void do_keyboard(unsigned char c)
{
	...
	if(c != 'x')
	{
		char buf[2];  // 创建一个长度为2的字符数组

		buf[0] = c;  // 将字符赋值给数组的第一个元素
		buf[1] = '\0'; // 终止符，表示字符串结束
		printk(buf);  // 调用printk函数打印字符
	}
	else{
		printk("\n");
		if(def == 1){
...
void keyboard_interrupt()
{
    ...
    if (c == 0xf0)
    {
        ...
        if(c == 0x58)
        {
            def = ! def; // 设置 `def` 为 0，表示没有按下 caps键
        }
        return;
    }
    // 检查扫描码是否是 caps，表示 caps 键按下
    if (c == 0x58)
    {
        def = ! def; // 设置 `def` 为 1，表示 caps键按下
    }
	do_keyboard(c);
}
