...
extern struct process *process[64];
int (*syscalls[])() = {
	sys_fork};
...
void do_timer()
{
	if (count == 0) 
	{
		...
		buf0[2] = 'd';
		buf0[3] = ':';
		buf0[4] = '0' + process[0]->pid;
		print_kernel(buf0);
		char buf[2];
		buf[0] = '\n';
		buf[1] = '\0';
		print_kernel(buf);
		char str0[] = "process_name: ";
		print_kernel(str0);
		print_kernel(process[0]->name);
		print_kernel(buf);
		char str1[] = "exe_end: ";
		char str2[] = "page_directory: ";
		print_debug(str1, process[0]->exe_end);
		print_debug(str2, process[0]->page_directory);
		buf0[0] = 'p';
		buf0[1] = 'i';
		buf0[2] = 'd';
		buf0[3] = ':';
		buf0[4] = '0' + process[1]->pid;
		print_kernel(buf0);
		print_kernel(buf);
		print_kernel(str0);
		print_kernel(process[1]->name);
		print_kernel(buf);
		print_debug(str1, process[1]->exe_end);
		print_debug(str2, process[1]->page_directory);
		count = 1;
	}
}
