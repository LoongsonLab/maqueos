...
void write_detail(int seq,int type,short blocknum,int used){
	if(step==0){
       printk_block_title();
	   step++;

	}
	write_char(seq+'0',dx,dydetail);
	write_char(type+'0',dx+14,dydetail);
	write_char(blocknum+'0',dx+23,dydetail);
	write_char(used+'0',dx+36,dydetail);
	dydetail++;
     return;
}