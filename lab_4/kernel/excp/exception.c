...
int (*syscalls[])() = {
	sys_fork, sys_input, sys_output, sys_exit, sys_pause,sys_drewPlane,sys_drewGplane,sys_drewBplane,sys_drewYplane};
...
void int_on()
{
	...
	crmd = read_csr_32(CSR_CRMD);//当前状态u寄存器
	write_csr_32(crmd | CSR_CRMD_IE, CSR_CRMD);//启用全局中断
}
void excp_init()
{
	unsigned int val;
