...
#define PWCL_PD2BASE 30
#define PWCL_PD2WIDTH  9
...
{
	unsigned long pd, pd2, pt;
	unsigned long *pde, *pd2e, *pte;

	pd = p->page_directory;
	pde = (unsigned long *)(pd + ((u_vaddr >> 30) & 0x1ff) * ENTRY_SIZE);
	if (*pde)
		pd2 = *pde | DMW_MASK;
	else
	{
	...
	pd2e = (unsigned long *)(pd2 + ((u_vaddr >> 21) & 0x1ff) * ENTRY_SIZE); 
	if (*pd2e)
		pt = *pd2e | DMW_MASK;
	else
	{
...
void mem_init()
{
	...
	write_csr_64((PWCL_EWIDTH << 30) | (PWCL_PD2WIDTH << 25) | (PWCL_PD2BASE << 20) | (PWCL_PDWIDTH << 15) | (PWCL_PDBASE << 10) | (PWCL_PTWIDTH << 5) | (PWCL_PTBASE << 0), CSR_PWCL);
	invalidate();
}